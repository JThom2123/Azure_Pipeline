...
export default function ITunesSearchPage() {
  ...
  async function handleSearch() {
    ...
    try {
      ...
      // For each song, check if there's a saved point value in localStorage, else default to 100
      const newResults = data.results.map((item: any) => {
        const storedPoints = getStoredPoints(item.trackId);
        return {
          ...item,
          points: storedPoints !== null ? storedPoints : 100, // Use stored points or default to 100
        };
      });
